...
% Copyright Ross L. Hatton, 2011
	...
	% is not present
	if ~ ( (ispc && strcmp(':\',destination{1}(2:3))) || (isunix && strcmp('/',destination{1}(1)) ) )
		
		destination = cellfun(@(x)fullfile(pwd, x),destination,'UniformOutput',false);
		...
		for i = 1:length(framepoints)
			...
			for j = 1:numel(export)
				...
				if export(j)
					...
					frame_info.printmethod(destination_str);
